// Tests for draft-selection.ts's own logic: the pure geometry functions directly, and the
// module's pointer handling through a hand-rolled `ClientSurface` double — no engine involved,
// since nothing here reaches `$`. `claude plugin test plugin`'s kit has no built-in way to
// drive a real `Client` instance's pointer events (checked: no such call on its `Engine` or
// `Mock` types), so this double is this plugin's own, not a kit feature.
...
import {
  ...
  selectedColumnsOf,
  visualRowsOf,
} from '../hooks/draft-selection'

describe('draft-selection geometry', () => {
...
// Drives draft-selection.ts's own module function the way the engine drives a real `Client`:
// `setState` re-runs it at once with the new state, which re-registers `onPointer` with a
// handler that closes over that fresh state — so a `fire` after a `setState` reaches the
...
function driveDraftSelection(lines: readonly string[], armedRange?: { start: number; end: number }, columns = Number.POSITIVE_INFINITY) {
...
describe('draft-selection pointer handling', () => {
  test('a click with no movement and nothing armed posts nothing', () => {
    const drive = driveDraftSelection(['line one', 'line two'])
  ...
  test('a click landing inside the armed range posts cleared: this is how a selection is dropped', () => {
    ...
    const drive = driveDraftSelection(['line one', 'line two'], { start: 0, end: 8 })
  ...
  test('a drag over Japanese text lands on the character under each cell, not one screen row per character', () => {
    const drive = driveDraftSelection(['こんにちは'])
  ...
  test('a drag over Japanese text still maps correctly across a wrap point', () => {
    ...
    const drive = driveDraftSelection(['こんにちは'], undefined, 6)
  ...
  test('move before any down is a hover, not a drag: nothing posted, no crash', () => {
    const drive = driveDraftSelection(['line one'])
  ...
  test('an active drag highlights only the covered run on each line', () => {
    const drive = driveDraftSelection(['abcdef'])
  ...
  test('a wrapped line still maps a drag to the right characters, across the wrap point', () => {
    ...
    const drive = driveDraftSelection(['abc def ghi'], undefined, 7)
  ...
  test('an already-armed range stays highlighted with no drag in progress', () => {
    const drive = driveDraftSelection(['abcdef'], { start: 1, end: 4 })
