...
2. When the draft already exists as a file, write the block with the header and one line
   `file: <absolute path>`, and no other lines. Do not copy the file's text into the transcript.
   For a revision of a file draft, edit the file, then post a new block with the new number,
   `(revises D<n>)`, and the same `file:` line.
3. Number drafts in one sequence for the conversation. Start after the highest `D<n>` already in
   the transcript. Never reuse a number.
4. Write a revision as a new block with a new number. Its first line is
   `D<m> (revises D<n>): <title>`. Never rewrite an earlier block in place: a later comment names
   "D3", and that must keep one meaning.
5. When the draft itself contains a three-backtick fence, open and close the `draft` block with
   four backticks.
6. Write what the draft is for, and any doubt about it, in plain text before the block. Write
   nothing that belongs to the draft outside the block: the pane shows only the block.
7. A prompt that starts with `Feedback (draft-pane) on D<n>:` is the person's feedback on that
   ...
   next step with that text. For any other feedback, write the revision as in step 4, in the same
   turn.
